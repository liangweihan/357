...
def scrape_stock_data(stock_code):
    ...
    def scrape_dividend_data():
        url = f'https://tw.stock.yahoo.com/quote/{stock_code}.TW/dividend'  # Yahoo股票網頁的URL
        response = requests.get(url)  # 發送HTTP GET請求取得網頁內容
        ...
        try:
            ...
            for i in range(0, len(all_cash_dividends), 4):
                chunk = all_cash_dividends[i:i+4]  # 每四個資料為一循環

                # 前兩個資料若有不是數值的以0取代，後面兩個資料也以0取代，然後將四個資料加總
